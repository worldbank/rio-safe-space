...
*******************************************************************************/
	...
	* Any phase 2 rides, controlling for baseline takeup, if at least 5 rides at 0 OC
	...
	sum		d_anyphase2 										if baselinerides >= 5
	estadd 	scalar 	mean1 	= round(r(mean), .001)

	* Any phase 3 rides, controlling for baseline takeup, if at least 5 rides at 0 OC
	reg d_anyphase3  $demographics pcbaselinetakeup 			if baselinerides >= 5
	est sto attrition4
	
	sum d_anyphase3 											if baselinerides >= 5
	estadd scalar	mean1 	= round(r(mean), .001)
	
	* Any phase 3 rides, controlling for baseline takeup and takeup at 5/10 cents, if at least 5 rides at 10 or 5 OC
	reg d_anyphase3 $demographics pcbaselinetakeup any510takeup if fivetenrides >= 5 
	est sto attrition5
	
	sum d_anyphase3 											if fivetenrides >= 5 
	estadd scalar 	mean1 	= round(r(mean), .001)
...
*******************************************************************************/
	...
	esttab attrition1 attrition3 attrition2  attrition4 attrition5 ///
		using "${out_github}/attrition.tex", ///
		...
		mgroups("{p{3.2cm}}{\center Started revealed preferences rides" "{p{5cm}}{\center Started randomized car assignment rides", pattern(1 0 1 0 0) prefix(\multicolumn{@span}) suffix(}) span erepeat(\cmidrule(lr){@span})) ///
		prefoot(" \\[-1.8ex] \hline  \\[-1.8ex]") ///
		postfoot("\hline\hline \end{tabular} ") ///
		posthead("\hline \\[-1.8ex]")

	copy "${out_github}/attrition.tex" "${out_tables}/attrition.tex", replace
